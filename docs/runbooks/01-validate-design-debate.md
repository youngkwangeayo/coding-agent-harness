...
- [ ] **반박 라운드(조건부)**: 구현자가 **P1에 반론**으로 맞선 경우에만 그 contested P1이 `design-reviewer`로 1회 되돌아가 유지·보강/인정 응답이 나오는지. 반론이 없으면 이 단계가 **건너뛰어지는지**(항상 도는 게 아님). 선독립 가드 — 리뷰어가 *수사*가 아닌 *새 증거*에만 굽히는지.
- [ ] `design-arbiter`가 **발견+응답(+있으면 리뷰어 반박)**을 항목별 판정(고친다/유지+ADR씨앗/바꾼다)함.

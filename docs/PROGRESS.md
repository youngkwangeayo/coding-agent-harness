...
**조건부 반박 라운드 추가** (2026-06-12). 실제 토론 1회(클러스터 통합 주제) 후 사용자 지적 — "리뷰어↔구현자 직접 토론이 없다"는 비대칭. 구현자가 **P1에 반론**으로 맞선 contested 항목에 한해, 리뷰어에 **1회 반박 기회**를 주는 조건부 라운드를 SKILL 절차 4단계로 삽입(반론 없으면 건너뜀). 선독립 가드 유지 — 리뷰어는 *새 증거*에만 굽히고 수사엔 안 접음. `SKILL.md`(원칙1·절차)·`design-reviewer.md`(반박 모드 절)·런북 검증항목 수정. **정적 정합만 확인, e2e는 새 세션에서 contested P1 케이스로 미검증.**
